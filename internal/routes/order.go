...
func OrderRoutes(incomingRoutes *gin.Engine, q *queries.Queries, redisClient *db.RedisConfig, matchingConfig *matching.MatchingEngine) {
	...
	// incomingRoutes.GET("/orders/:id", controllers.GetOrder(q))
	...
	incomingRoutes.PUT("/cancel_order/:id", middleware.VerifyUser(q), controllers.CancelOrder(q, redisClient)) // TODO: add auth middleware
}
